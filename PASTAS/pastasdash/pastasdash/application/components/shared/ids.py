"""File containing component IDs for the Dash app."""

# HEADER
HELP_BUTTON_OPEN = "help-button-open"
HELP_BUTTON_CLOSE = "help-button-close"
HELP_MODAL = "help-modal"
HELP_TITLE = "help-title"
LOAD_PASTASTORE_BUTTON = "load-pastastore-button"

# ALERT
ALERT = "alert"
ALERT_DIV = "alert-div"
ALERT_BODY = "alert-body"

# ALERT STORES
ALERT_TAB_RENDER = "alert-tab-render"
ALERT_TIME_SERIES_CHART = "alert-time-series-chart"
ALERT_PLOT_MODEL_RESULTS = "alert-plot-model-results"
ALERT_SOLVE_MODEL = "alert-solve-model"
ALERT_SAVE_MODEL = "alert-save-model"

# STORES
SELECTED_OSERIES_STORE = "selected-oseries-store"
PASTAS_MODEL_STORE = "pastas-model-store"
DOWNLOAD_MAP_DATA_STORE = "download-map-data-store"
PASTASTORE_CONFIG_FILE_STORE = "pastastore-config-file-store"

# TABS
TAB_CONTAINER = "tab-container"
TAB_CONTENT = "tab-content"
TAB_OVERVIEW = "tab-overview"
TAB_MODEL = "tab-model"
TAB_COMPARE = "tab-compare"
TAB_MAPS = "tab-maps"


# OVERVIEW TAB
OVERVIEW_MAP = "overview-map"
OVERVIEW_TABLE = "overview-table"
LOADING_SERIES_CHART = "loading-series_chart"
SERIES_CHART = "series-chart"
OVERVIEW_CANCEL_BUTTON = "overview-cancel-button"
# Customization controls
OVERVIEW_LAYOUT_MODE = "overview-layout-mode"            # overlay | stacked
OVERVIEW_STRESS_CHECKLIST = "overview-stress-checklist"  # [prec, evap]

# MODEL TAB
MODEL_DROPDOWN_SELECTION = "model-dropdown-selection"
MODEL_DATEPICKER_TMIN = "model-datepicker-tmin"
MODEL_DATEPICKER_TMAX = "model-datepicker-tmax"
MODEL_SOLVE_BUTTON = "model-generate-button"
MODEL_SAVE_BUTTON = "model-save-button"
MODEL_RESULTS_CHART = "model-results-chart"
MODEL_DIAGNOSTICS_CHART = "model-diagnostics-chart"
MODEL_USE_ONLY_VALIDATED = "model-use-only-validated-checkbox"

# COMPARE TAB
COMPARE_MAP = "compare-map"
COMPARE_METADATA_TABLE = "compare-metadata-table"
COMPARE_PARAMETERS_TABLE = "compare-parameters-table"
COMPARE_CHECKS_TABLE = "compare-checks-table"
COMPARE_MODEL_SELECTION_DROPDOWN = "compare-model-selection-dropdown"
COMPARE_MODELS_CHART = "compare-models-chart"
COMPARE_TABLE_SELECT_ALL_BUTTON = "compare-table-select-all-button"
COMPARE_TABLE_DESELECT_ALL_BUTTON = "compare-table-deselect-all-button"

# MAP TAB
MAP_DROPDOWN_SELECTION = "mapresult-dropdown-selection"
MAP_RESULT_FIGURE = "map-result-figure"
MAP_COLORMAP_SELECTION = "map-colormap-selection"
MAP_RENDER_BUTTON = "map-render-button"
REVERSE_COLORMAP_CHECKBOX = "reverse-colormap-checkbox"
DOWNLOAD_MAPDATA_BUTTON = "download-mapdata-button"
DOWNLOAD_MAPDATA = "download-mapdata"
MAPDATA_CMAP_MIN = "mapdata-cmap-min"
MAPDATA_CMAP_MAX = "mapdata-cmap-max"


# DUPLICATE CALLBACK STORES
OVERVIEW_TABLE_SELECTION_1 = "overview-table-selection-1"
OVERVIEW_TABLE_SELECTION_2 = "overview-table-selection-2"
